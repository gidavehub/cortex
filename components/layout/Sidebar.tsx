...
import { LayoutDashboard, Calendar, Settings, LogOut, Target, Trophy, GitBranch } from "lucide-react";
...
export function Sidebar() {
    ...
    return (
        <aside className="fixed left-6 top-1/2 -translate-y-1/2 z-40">
            <div className="glass flex flex-col items-center py-6 px-3 rounded-full shadow-2xl bg-white/90 backdrop-blur-xl border border-white/40">
                {/* Cortex Logo - Pure SVG, No Background */}
                <div className="mb-8">
                ...
                <nav className="flex flex-col gap-4">
                    {navItems.map((item) => {
                        const isActive = pathname === item.href ||
                            (item.href === "/dashboard/calendar" && pathname?.startsWith("/dashboard/calendar")) ||
                            (item.href === "/dashboard/tasks" && pathname?.startsWith("/dashboard/tasks")) ||
                            (item.href === "/dashboard/conditionals" && pathname?.startsWith("/dashboard/conditionals")) ||
                            (item.href === "/dashboard/achievements" && pathname?.startsWith("/dashboard/achievements"));
                        return (
                            <Link
                                key={item.href}
                                href={item.href}
                            ...
                            >
                                ...
                                {/* Tooltip */}
                                <span className="absolute left-full ml-4 px-3 py-1.5 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none shadow-lg">
                                    {item.label}
                                </span>
                            </Link>
                        );
                ...
                <div className="w-8 h-px bg-gray-200 my-6"></div>

                {/* Logout */}
                <button
                    onClick={() => logout()}
                    className="w-12 h-12 flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors group relative"
                >
